	...
	function submissions($args) {
		...
		if ($export === "PDF"){
			...
			$spreadsheet->getProperties()
				...
				->setTitle($schedConf->getLocalizedTitle() . " export")
				->setSubject($page)
				->setDescription("Export generated by the conference editorial manager");
			$locale = 'cs';
	...
	function renderSpreadsheet(&$spreadsheet, $page = false, $submissionsArray){
		...
		if ($page == "submissionsUnassigned"){
			...
			$spreadsheet->setActiveSheetIndex(0)
				...
				->setCellValue($column++ . $row, __('paper.authors'))
				->setCellValue($column++ . $row, __('paper.title'))
				//->setCellValue($column++ . $row, __('submissions.reviewStage'))
				->setCellValue($column++ . $row, __('common.status'))
				->setCellValue($column++ . $row, __('common.date'));
			...
			foreach($submissionsArray as $submission){
				...
				->setCellValue($column++ . $row, $submission->getAuthorString(true))
				->setCellValue($column++ . $row, $submission->getLocalizedTitle())
				//->setCellValue($column++ . $row, $submission->getStage())
				//->setCellValue($column++ . $row, $decisionTag)
				->setCellValue($column++ . $row, $status)
				->setCellValue($column++ . $row, $date);
			}
			// Formatting
			$spreadsheet->getActiveSheet()->getStyle('E2:E' . $row)
				->getNumberFormat()
				->setFormatCode('DD.MM.YYYY');
			$spreadsheet->getActiveSheet()
				->getColumnDimension('A')
			...
			$spreadsheet->setActiveSheetIndex(0)
				...
				->setCellValue($column++ . $row, __('paper.authors'))
				->setCellValue($column++ . $row, __('common.country'))
				->setCellValue($column++ . $row, __('user.affiliation'))
				->setCellValue($column++ . $row, __('paper.title'))
				->setCellValue($column++ . $row, __('submissions.reviewStage'))
				->setCellValue($column++ . $row, __('submission.decision'))
				->setCellValue($column++ . $row, __('user.role.trackDirectors'));
			$spreadsheet->getActiveSheet()->getStyle('A1:'. $column . $row)
				->getFont()->setBold(true);
			$spreadsheet->getActiveSheet()->getStyle('A1:'. $column . $row)
				->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
			...
			foreach($submissionsArray as $submission){
				...
				$spreadsheet->getActiveSheet()
					...
					->setCellValue($column++ . $row, $submission->getAuthorString(true))
					->setCellValue($column++ . $row, $user->getCountry())
					->setCellValue($column++ . $row, $user->getAffiliation())
					->setCellValue($column++ . $row, $submission->getLocalizedTitle())
					->setCellValue($column++ . $row, $submission->getCurrentStage() - 1)
					->setCellValue($column++ . $row, $decisionTag)
					->setCellValue($column++ . $row, $submission->getTrackDirectorString(true));
			}
			// Formatting
			$spreadsheet->getActiveSheet()
				->getColumnDimension('A')
